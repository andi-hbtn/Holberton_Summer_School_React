import { Controller, Get, Post, HttpException, Body, HttpCode, UseInterceptors, UploadedFiles } from '@nestjs/common';
...
export class GameController {
	...
	@UseInterceptors(AnyFilesInterceptor(
		{
			storage: diskStorage({
				...
				filename: (req, file, collback) => {
					...
					//console.log("file--create--", imageName)
	...
	public async create(@Body() bodyParam: any, @UploadedFiles() file: Express.Multer.File[]) {
		try {
			return null
			// const result = await this.gameService.createGame(bodyParam, file[0].path);
			// return result;
		} catch (error) {
			console.log("error in get all testimonials", error);
			throw new HttpException('Un error has been encountered during the get', 503)
